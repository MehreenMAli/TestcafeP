...
var portalFixture = fixture`Level 1 - Portal - Creation - Running on "${args.env.toUpperCase()}"`
...
test(`TC 27173: Create a new Table`, async t => {
    ...
    const tableName = '27173_TestTable';
    ...
    let tables = await apiHandler.getCustomerTables(corcentricCustomer.client_id);
    try {
        //Check saved table.
        tables = (await apiHandler.getCustomerTables(corcentricCustomer.client_id)).items;
        let testTable = tables.find(element => element.table_name === tableName);
        tableIndex = tables.indexOf(testTable);
...
test(`TC 27325: Duplicate Table Error`, async t => {

    const glTablePage = new GLTablePage();
	let customers = (await apiHandler.getAllCustomers()).items;
    const customerName = 'Corcentric';
    const tableName = '27325_TestTable';
    //2. Go to Portal
    await t
